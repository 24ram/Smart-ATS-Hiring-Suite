...
export default function PipelinePage() {
  ...
  const stageMutation = useMutation({
    mutationFn: ({ id, status }: { id: string; status: Application["status"] }) => applicationService.updateStatus(id, status),
  ...
  const onDragEnd = (result: DropResult) => {
    ...
    const newStage = destination.droppableId as Application["status"];
                ...
                <Droppable droppableId={stage.id}>
                  {(provided, snapshot) => (
                    <div
                      ...
                      className={`p-3 flex-1 overflow-y-auto space-y-3 min-h-[150px] transition-colors ${snapshot.isDraggingOver ? 'bg-blue-50/50 dark:bg-blue-900/20' : ''
                        }`}
                    >
                        ...
                        <Draggable key={application.id} draggableId={application.id} index={index}>
                          {(provided, snapshot) => (
                            <div
                              ...
                              className={`bg-white dark:bg-gray-800 p-4 rounded-lg border transition-all ${snapshot.isDragging
                            ...
                            >
                              ...
                              <div className="pl-6">
                                {application.ai_score !== undefined && application.ai_score !== null && (
                                  <div className="mb-3">
                                    <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-[11px] font-semibold ${application.ai_score >= 90 ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400' :
